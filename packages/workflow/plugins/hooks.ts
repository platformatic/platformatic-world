...
async function hooksPlugin (app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/v1/apps/:appId/hooks', async (request) => {
    ...
    // correlation_id is the SDK-generated monotonic ULID assigned in workflow
    // source order. Sort by it primarily — concurrent hook_created POSTs land
    // in arbitrary DB insertion order, so created_at (microsecond precision)
    // would otherwise put the second-source-call before the first.
    const result = await app.pg.query(
