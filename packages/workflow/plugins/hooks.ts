...
async function hooksPlugin (app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/v1/apps/:appId/hooks', async (request) => {
    ...
    // Three hooks created synchronously inside one workflow invocation can
    // share created_at at millisecond precision, so tie-break on
    // correlation_id (a monotonic ULID generated by the SDK) to preserve
    // workflow-creation order. Without this, tests like webhookWorkflow
    // see hooks in arbitrary order.
    const result = await app.pg.query(
